...
def CleanScan():
    ...
    camera.capture('cleanwatertest.jpg')
    scan = cv2.imread('cleanwatertest.jpg',cv2.IMREAD_UNCHANGED) 
    ...
    blurred1 = cv2.GaussianBlur(grey, (0, 0), cv2.BORDER_DEFAULT)
    blurred = cv2.blur(blurred1, (8, 4), cv2.BORDER_DEFAULT)
    thresh = cv2.threshold(blurred1, 117,255, cv2.THRESH_BINARY)[1]
